...
- The Reading Room: magazines, books and catalogs from the Media Pack and the
  GLP add-on, streamed on demand. Game details link to their articles.
- PDFs open in-app on every platform, including Linux. Manuals too.
- Settings → Storage: disk usage by category with cleanup buttons, and every
  installed game by size with uninstall.
- Option to drop a game's ZIP after install (Settings → Storage). Reset then
  downloads it again.
- The game details show the game's size on disk.
- HD cover pack for the Reading Room.

### Changed

- Uninstall keeps only the files you changed or added, not the whole folder.
- Settings has a sidebar and one consistent row layout.
- DOSBox Staging 0.83.0.

### Fixed

- Windows: uninstall could leave the game folder behind and silently copy it
  into the save backup. A failed delete is now an error and the game stays
  installed.
