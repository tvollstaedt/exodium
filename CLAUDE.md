...
│       │   ├── reading.rs     ← publications, issues, articles, reading state
...
│   ├── ScummVM.xml.gz         ← eXoScummVM catalog (platforms "ScummVM" + "ScummVM SVN")
│   ├── media.json.gz          ← Lesesaal catalogue: magazines, books, catalogs
│   ├── media_articles.txt     ← eXo's per-game magazine index (shortcode → page)
│   ├── scummvm.txt            ← eXo's launch index: dir → engine:gameid → pinned build
│   ├── dosboxsvm.txt          ← dir → build slug, feeds dosbox_variant for eXoScummVM
│   └── scummvm_ini/<build>.ini ← eXo's per-build scummvm.ini (targets + defaults)
...
│   ├── eXoWin3x.torrent
│   ├── eXoScummVM.torrent
│   └── eXoDOS Media Pack.torrent  ← magazines, books, catalogs (§19)
...
│   └── gen_scummvm_assets.py  ← rebuild the bundled eXoScummVM catalog, index and inis
...
There is no per-collection opt-in UI (the old `Intro.tsx` was removed in 0.14). `Setup.tsx` always enables all available collections. If per-collection opt-in is reinstated, the key invariant is: `setConfig("collections", ...)` must be called and awaited **before** `initDownloadManager()`.
...
differently. It strips a trailing BACKSLASH before `stat()`ing a mount target
("Removing trailing backslash if not root dir so stat will succeed") - Windows
only, matching `\` alone, so our forward slash walks past it and the mount
fails: `mount c .\eXoDOS\` is how 1,570 eXoDOS configs (DOOM II, Relentless)
...
upgrade. There is no per-collection opt-in UI,
...
eXoDOS's util.zip). `support_files.rs` owns that pipeline for every such
payload (`SupportPack`: util suffix, inner zip, prefixes, readiness): queued
with the first game that needs it (disk preflight +8 GB here), a watcher
extracts `emulators/dosbox/` + `emulators/86Box98/` (NOT PCBox/audio) with
staging + atomic renames, and `init_download_manager` re-arms every pack's
watcher after restarts. Readiness =
...
won't open; `extract_before_launch` (the one launch pre-step for every
collection) therefore reports `ExtractError::NotAnArchive` as "incomplete or
...
### 18. eXoScummVM: one command line per game, seven pinned ScummVM builds

832 games (v2026.1.0, Feb 2026), 264 GiB torrent, infohash
`8d490e5c405dc30161c16bd907372ac01523f5c7`. Same pack shape as eXoWin3x -
flat title-named zips at `eXo/eXoScummVM/<Title (Platform)>.zip` - and like
eXoWin9x it has no shortcodes: the zip stem IS the title directory and the
catalogue key (`shortcode_segment: "!ScummVM"`, `Launcher::ScummVm`). The
`launcher` field on `CollectionDef` is what `launch_game` and `download_game`
dispatch on; path derivation still keys on `year_subdirs`/`lang_dir`.

**There is no per-game conf.** eXo runs every title through ONE bat
(`util/launch_svm.bat`) that looks the game up in `util/scummvm.txt`
(`<dir>;<engine:gameid | target>;<build>\scummvm.exe`) and assembles a
single ScummVM command line. `commands/scummvm.rs` does the same from the
bundled copy of that index (`metadata/scummvm.txt`): none of §10a's mount
rewriting applies, and there is nothing to patch. Three things are easy to
get wrong:

- **`--no-console` is a Windows-only option.** The macOS and Linux binaries
  reject it outright ("Unrecognized option"), so it is added on Windows only.
- **Unrecognised game data does NOT fail the launch.** ScummVM logs a warning
  and opens its own launcher GUI, which from Exodium's side looks like a game
  that started. `detect()` therefore runs `--detect --path=<dir>` first and
  refuses when the expected `engine:gameid` is not among the rows; a
  placeholder or piece fragment (§16, `zip.exists()` is not "downloaded")
  lands there with an "incomplete" message instead of a blank launcher.
- **Both spawns need fork+exec on macOS.** `Command::output()` for the detect
  probe hit the same posix_spawn EBADF as the emulator spawn
  (`spawn_emulator_and_track`); the no-op `pre_exec` is not optional.
- **`--config` wants `--config=<path>`**, not two arguments.

**Seven pinned builds, and the pins are real.** eXo keeps old ScummVM
versions because games break across releases (its readme: "5 or 6 old
versions"); `metadata/dosboxsvm.txt` records the pin per game in
`dosbox_variant` (slug = eXo's build dir: `default` = the root
`scummvm.exe`, which is 2.8.0 despite its ini saying 2.6.1; `2.5`, `2.9.0`,
`2026.1.0`; and three git snapshots whose version strings carry the commit
hash - `BUILDS` in scummvm.rs maps slug to version). Distribution: 2.9.0
387 index entries, 2026.1.0 287, 2.8.0 259, the rest 20. Resolution order
(`resolve_scummvm`): eXo's own build under `eXo/emulators/scmvm/<slug>/`
...
folder per platform (`Maniac Mansion (Multi-Platform)` holds nine - Amiga,
C64, NES, DOS v1, DOS v2, Deluxe, ...), `platform.txt` gives `--platform`,
`menu.txt` marks a second level (`video.txt` = `--render-mode` for CGA/EGA/
Hercules), `sound.txt` lists the sound menu (Sound Blaster / Roland MT32 /
Sound Canvas / Tandy / CMS; the choice picks the music driver and a
`<prefix>audio.txt`), `subtitle.txt`, `audio.txt`, `note.txt`, and an
`Other Languages` folder. `select_variant` walks it the way the bat does and
takes stored choices from `game_config` (`svm_variant`, `svm_sub`,
`svm_sound`, `svm_subtitles`, `svm_aspect`), defaulting to the first entry
at every level like eXo's menu. The index lookup goes innermost dir first,
then the zip stem, exact before prefix - `findstr /b` semantics, since 122
index lines name inner variants and most variant dirs have none. MT-32 ROMs
and the SoundCanvas soundfont sit in `utilSVM.zip` (`mt32/`), so those two
sound choices fall back to Sound Blaster with a log line until that is
fetched. Variants are deliberately NOT catalogue rows: 833 would become
1,500+, one game would render as several cards, and the tree is only known
after unpacking - eXo keeps one entry per zip too.

**Saves go to `<game dir>/!saves`** (`--savepath`), so uninstall's rename
backup and Reset (§5) carry them; ScummVM's default is a per-user folder
outside the library. Each pinned build's ini (eXo's `[scummvm]` defaults plus
the target sections 44 games launch by name, e.g. `kq6-cd-win`, `lol-cd`) is
copied once from `metadata/scummvm_ini/` to `content/scummvm/<slug>.ini`,
writable, because ScummVM writes its own settings back into the file it was
started with.

**Two LaunchBox platforms, one collection.** eXo files its "may be unstable"
titles under platform `ScummVM SVN` (109 rows) beside `ScummVM` (724);
`gen_scummvm_assets.py` merges both XMLs into `ScummVM.xml.gz` - all `<Game>`
blocks first, then the rest, because the serde importer reads each element
kind as one contiguous run and rejects a `<Game>` after an `<AlternateName>`
as "duplicate field". `games.platform` keeps the original value, which is
what the panel's System tag shows and what `gen_thumbnails.py --platform`
scopes on (two passes into `thumbnails/eXoScummVM/`). The same script
re-cases the `!ScummVM\<dir>` in three rows to match their zip
(`Escape From Hell (DOS)` vs `Escape from Hell`): the extracted folder
follows the zip, the catalogue's dir is the shortcode, and installed-game
matching compares them exactly - on Linux the game would extract and still
read as not installed.

No `Content/GameData`, no videos; 68 theme tracks and 333 manuals live in
`XOScummVMMetadata.zip` (a future metadata pack), the 699 PDF/UHS extras in
`!ScummVMmetadata.zip` (7.4 GB). `gamedata_torrent_index` is NULL on every
row, so `music_file` is nulled at import (§14's pipeline has nothing to
fetch here).

**Verified 2026-09-06 on macOS** (dev build): catalog upgrade 11 -> 12 with
...
### 19. The Media Pack: a reading room, not a collection

The eXoDOS **Media Pack** is its own torrent (236.8 GB, 9 files, 8 MiB pieces,
infohash `cb73d61e6b4ea5eaa793c64c1d80e6113e95de84`,
`https://www.retro-exo.com/eXoDOS%20Media%20Pack.torrent`). Three of its files
carry documents: `Content/DOSMagazines.zip` (102.0 GB), `Content/DOSBooks.zip`
(45.0 GB), `Content/DOSCatalogs.zip` (1.7 GB). 1,300 PDFs in 30 publications
plus 241 runnable disk magazines - and, from a second source, 654 German
magazines (below) - the Lesesaal, a third tab beside Browse and My Library.

**It is deliberately NOT a `CollectionDef`.** Every collection surface is a
projection of `COLLECTION_MAP` (§15), and `enable_new_collections` appends each
new id to the user's `collections` config - a Media Pack entry would hand a
237 GB torrent to every existing install on the next start, for a "collection"
in which nothing can be installed or launched. It lives in `MEDIA_SOURCES`
(`src-tauri/src/media_sources.rs`) instead, and its `DownloadManager` is
created on the FIRST issue opened, on the session the collections already run
(`ensure_manager`). Offline there is no session, hence no manager, hence no
Lesesaal downloads - browsing still works, because the catalogue is bundled.

**The first issue opened lands on a torrent that is not readable yet.**
librqbit answers `invalid state: initializing` for a moment after the add, so
`open_stream` waits it out instead of surfacing that as an error - measured in
the field, the first click failed and the second one worked. No fastresume
seed is needed for it: the initial check reports "total selected 0" and
returns in milliseconds, because this source never selects a file at all.

**A new torrent joining the shared root must re-union the cleanup keep-list.**
All torrents write into one root and `cleanup_placeholder_files` deletes what
is not on that list, so `ensure_manager` recomputes it across the collection
managers plus the new one. Without that the next cleanup sweeps the Media
Pack's files out from under a running stream.

**The catalogue is bundled, the archives are not.** `metadata/media.json.gz`
(166 KB) and `metadata/media_articles.txt` are generated by
`scripts/gen_media_assets.py` from the pack's own `Data/Platforms/*.xml` -
normal LaunchBox documents, so `Title`, `ReleaseDate`, `Publisher`, `Notes`
and `Genre` (the publication: `Magazine / PC World`, `Books / Programming`,
`Software Catalog`) come straight from eXo. `generate_db` fills
`publications`, `issues` and `game_articles`; those three are pure catalog and
`refresh_catalog` replaces them WHOLE (`copy_catalog_tables`), which is only
safe because the user's state lives in `issue_state`, keyed by `issues.key`
rather than by a row id.

**The index carries no zip offsets, and must not.** The reader has the live
central directory anyway - read once per archive per session and cached
(`ReadingState.directories`; 48,897 entries for the magazines archive, 7.2 s
over a warm swarm) - and a repack on eXo's side would invalidate baked
offsets while leaving the names correct.

**Issues stream to disk, not through memory.** `read_entry_with` caps an entry
at 512 MiB and returns a `Vec<u8>`; books run to 222 MB median and 1.49 GB
compressed at the top, 13 of them over that cap. `read_entry_to_writer` is
the Lesesaal's road: chunked, inflating as it goes, into
`content/magazinecache/<hash of key>.<ext>` via a `.part` rename. **A download
stays until the user removes it**, like a game: nothing sweeps the cache, and
`remove_issue` (reader header, grid context menu) is the only way a document
or an extracted disk magazine leaves the disk - it takes the issue's own
directory, never a series' shared `launch_dir`, and refuses a path outside the
data dir. Startup sweeps `.part` leftovers alone, or a fetch killed mid-write
is served as a truncated document forever; `reading_storage_usage` is what
Settings shows for the space. Progress comes from the session's
`file_progress`, not from the reader, for the reason in §14.

**Rendering is pdf.js, on all three platforms.** WebKitGTK has no PDF viewer
at all, so the old `<iframe>` was a blank page on the primary target (#30).
`PdfReader.tsx` renders to canvas, releases a page's bitmap when it leaves the
viewport (a magazine runs to hundreds of pages at a few MB each), and is
loaded lazily - pdf.js plus its worker are ~2.5 MB and have no business in the
startup bundle. Three CSP entries make it legal: `'wasm-unsafe-eval'` in
`script-src` (the JPX/JBIG2 decoders), `worker-src 'self' blob:` and
`http://127.0.0.1:*` in `connect-src` (Linux serves the file through the
localhost media server, §14).

**`pdfjs-dist` is pinned to 5.6.205, and the pin is about WebKit's WebAssembly,
not about pdf.js.** Internet Archive scans (every PC World, 600 dpi, two JPX
images plus a JBIG2 mask per page) decode in pdf.js's OpenJPEG WASM module,
and from 5.7 that module is built with relaxed SIMD, which JavaScriptCore
rejects (`CompileError: relaxed simd instructions not supported` - measured
on macOS 26.6 / WebKit 21624 in a bare WKWebView, so WebKitGTK is the same
engine and the same answer). pdf.js then silently loads
`openjpeg_nowasm_fallback.js`, and a page takes minutes instead of a second:
the reader shows empty page boxes forever, with no error anywhere. The
`'wasm-unsafe-eval'` CSP entry and the WASM staging fixed JBIG2 (its module
compiles on every version), which is why the white pages went away while the
JPX pages stayed blank. `scripts/copy-pdfjs-wasm.mjs` scans the staged
modules for the relaxed-SIMD opcodes and fails the build on an upgrade that
...
each naming a PDF and a page; eXo opens SumatraPDF under Wine on it, we jump
the reader there. 486 of 487 shortcodes match `games.shortcode` exactly - the
one miss is eXo's typo `caeser2`, mapped in the generator. Kinds: Review,
Story, Preview, Walkthrough, Cheats, Hints.

**241 magazine issues are programs, not scans** (`issues.runnable`): disk
magazines with their own `dosbox.conf`, launched through the normal DOSBox
pipeline. Their files are contiguous in the archive, so a subtree is fetched
in one forward pass rather than entry by entry (`read_subtree_to_dir`: sorted
by local header offset, each entry via a `.part` rename). Big Blue Disk is one
210 MB directory for all 146 issues, selected by eXo's `CommandLine`; the
placeholder substitutions its launcher performs (`XXX`, `YYY`, `ZZZ`) are
derived from eXo's own scripts at generation time, not hardcoded.

`launch_issue` runs `<launch_dir>/dosbox.conf` through `patch_dosbox_conf`
with cwd `<game_root>/eXo`, which is where eXo's own launcher stands when it
starts DOSBox - the mounts in those confs are `.\Magazines\…` relative to it.
**The placeholders are filled into a DERIVED `run.bat`, never into the
original.** eXo substitutes in place and restores from a backup after the
emulator exits, so a crash leaves the file pinned to one issue; the pristine
text is kept as `run.exotpl` and `run.bat` is rewritten from it before every
launch.

**Disk magazines run on Staging everywhere, and that is unverified.** eXo's own
scripts pin DOSBox ECE (r4301, r4482 for the PC Gamer discs), but an issue
carries no `dosbox_variant`, so `resolve_engine(None, …)` answers Staging - on
Windows too, where a game would get ECE. Harvesting the pin is possible (the
`.bsh` names the build), but the Windows tree only ships ece4230/ece4460, so
it would resolve to a build that is not there. Nothing here has been launched
yet: the Windows `dosbox.conf` and `run.bat` were never read, only their Linux
twins.

**A size on a card is what THIS issue costs, and the series launchers make
that non-obvious.** Big Blue Disk selects one of 146 issues inside a single
210 MB directory by launcher argument, so deriving the size from `launch_dir`
gave every issue the whole series - 15.6 GB on each of the 25 Interactive
Entertainment CD issues. `issue_dir` is resolved per issue at generation time
(the subdirectory eXo's argument names, or the one whose name ends in it:
`BBD/004`, `GameBytes/gbytes14`), and an install takes that directory plus the
series' loose files, never the siblings. `generate_db` warns when a series'
issues all claim one size. What remains large is real: a book is a 200-400
page colour scan, and a PC Gamer cover disc is a CD image.

**The second source is the GLP torrent's magazine add-on, and it is a zip in
a zip.** `Content/eXoDOS_GLP_Addonpack_MagazinesGLP.zip` (60.4 GB, a fifth of
that torrent) is eXo's installer: a bat, `CHOICE.EXE`, two readmes and ONE
STORED entry, `Content/eXoDOS_GLP_Addonpack_MagazinesGLP_1.0.zip`, which holds
the 654 PDFs (9 series, 14 publications once eXo's Genre splits off the
specials - `ASM Sonderheft (DE)`, `PC Spiel Special (DE)` - median 86 MB), 654 covers, 654 LaunchBox `<Game>`
fragments under `xml/!german/magazines/` (bare elements, no XML root) and
9,697 game-to-page launcher bats - ten times the Media Pack's index, all nine
series covered. `issues.source` names the torrent, `issues.inner_zip` the
stored entry; the manager is the GLP collection's own from `TorrentState`, so
`media_sources.rs` stays the registry of non-collections. Every reading-room
stream is wrapped in an `OffsetReader` (a full-file window is the identity), and
the cached directory remembers `base`/`len`, or each fetch would re-read the
outer directory to find the inner archive. Two eXo quirks the generator
absorbs: the bats name eXo's INSTALLED path (`Magazines\!german\...`) while
the archive files the PDF under `eXo/Magazines/!german/...`, and special
issues are zero-padded in the file name but not in the title (`ASM Sonderheft
1` vs `ASM Sonderheft 01.pdf`) - the file stem is the canonical title. A
missing GLP manager while the session is up is reachable only by hand-editing
`collections` (there is no per-collection opt-in), so it is an `Err` from the
command, not a UI state; the panel's existing error path shows it and asks
again on the next open. Language is a column (`EN`/`DE`, the casing of
`games.language`), not a directory: all covers live in the one
`previews/eXoMedia/` set and resolve through `MEDIA_SOURCE`, never
`issue.source` - `previewDirs["eXoDOS_GLP"]` is the games' art.

Opening the first issue says once that reading joins a second torrent and that
seeding shares it (`media_notice_seen`) - information, not a second consent:
the seeding decision itself is §11's and lives in Settings → Network. The
gate and the dialog live in `MediaNotice.tsx` and BOTH entry points go
